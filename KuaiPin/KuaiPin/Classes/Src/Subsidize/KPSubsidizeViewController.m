...
- (void)didBottomToolClickCommitOrderButton:(KPSubsidizeBottomTool *)bottomTool {
    ...
    [KPNetworkingTool getReceiversWithParam:param success:^(id result) {
        ...
        if (receivers.count == 0) {
            ...
            orderCommitVc.popToSubsidizeVc = weakSelf;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    goodsDetailVc.activity_id = product.activityId;
    goodsDetailVc.virginUser = product.virginUser;
...
- (void)changeGoodNumber:(NSNotification *)notification {
    ...
    if ([notification.userInfo[GoodSpecIdKey] integerValue]) {
        param.productSpecId = notification.userInfo[GoodSpecIdKey];
    }

//    WHYNSLog(@"++++++++++%@+++++++++++++++", param.productSpecId);
