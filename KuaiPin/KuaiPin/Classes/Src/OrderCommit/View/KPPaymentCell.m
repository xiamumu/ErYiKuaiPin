...
@property (nonatomic, strong) KPPaymentButton *jdPayBtn;
...
- (void)setPaymentTag:(NSInteger)paymentTag {
    ...
    switch (paymentTag) {
        ...
        case AliPaymentTag:
            ...
            break;
        case JDPaymentTag:
            self.selectedBtn = self.jdPayBtn;
            break;
...
//    sepBottom.hidden = YES;
    ...
    [self.wePayBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(weakSelf);
        make.left.mas_equalTo(weakSelf.titleLB);
        make.top.mas_equalTo(weakSelf.balancePayBtn.mas_bottom);
        make.height.mas_equalTo(btnH);
    }];
...
//    }];
    ...
    [self.jdPayBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(weakSelf);
        make.left.mas_equalTo(weakSelf.titleLB);
        make.top.mas_equalTo(weakSelf.wePayBtn.mas_bottom);
        make.height.mas_equalTo(weakSelf.wePayBtn);
...
//    }];
    ...
    [sepBottom mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.height.mas_equalTo(sepTop);
        make.centerY.mas_equalTo(weakSelf.jdPayBtn.mas_top);
    }];
...
- (KPPaymentButton *)balancePayBtn {
    if (_balancePayBtn == nil) {
        ...
        [_balancePayBtn setTitle:@"零钱支付" forState:UIControlStateNormal];
...
- (KPPaymentButton *)jdPayBtn {
    if (_jdPayBtn == nil) {
        _jdPayBtn = [KPPaymentButton new];
        [_jdPayBtn setTitle:@"京东支付" forState:UIControlStateNormal];
        [_jdPayBtn setImage:[UIImage imageNamed:@"jd_pay"] forState:UIControlStateNormal];
        [_jdPayBtn addTarget:self action:@selector(changePayment:) forControlEvents:UIControlEventTouchUpInside];
        _jdPayBtn.tag = JDPaymentTag;
    }
    return _jdPayBtn;
}
