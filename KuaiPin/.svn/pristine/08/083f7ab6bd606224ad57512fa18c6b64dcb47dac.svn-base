...
#define BottomScrollViewWidth ScaleWidth(108)

@interface KPHomeCellBottomViewGoods ()<UIScrollViewDelegate>
...
@implementation KPHomeCellBottomViewGoods
...
- (void)setGoods:(NSArray *)goods
{
    ...
    for (int i = 0; i < count ; i++) {
        KPCommonView *commonView = [[KPCommonView alloc] initWithFrame:CGRectMake(commonView_X * i, margin, BottomScrollViewWidth, ScrollViewHeight)];
        commonView.commonViewType = KPCommonViewTypeTopImageWithBrandAndPrice;
...
//        commonView.backgroundColor = RandomColor;
    ...
    CGFloat moreBtn_H = ScrollViewHeight;
...
- (void)layoutSubviews
{
    ...
    self.bottomScrollView.frame = CGRectMake(0, CGRectGetMaxY(self.topScrollView.frame), SCREEN_W, ScrollViewHeight + 2);
}
