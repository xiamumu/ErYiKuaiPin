...
@interface KPBrandViewController ()<KPSortBarDelegate>
...
+(instancetype)shopViewControllerWithStoreId:(NSNumber *)storeId {
    KPBrandViewController *shopVc = [KPBrandViewController new];
...
- (void)addRightBarButtonItems
{
    ...
    UIBarButtonItem *collectionBtn = [UIBarButtonItem itemWithImageName:@"follow-off" highImageName:nil selectedImageName:@"follow-on" targe:self action:@selector(collectStoreWithCollectBtn:)];
    UIButton *btn = collectionBtn.customView;
    btn.selected = self.shopData.isCollection.integerValue;
    
    UIBarButtonItem *shareBtn = [UIBarButtonItem itemWithImageName:@"list_share" highImageName:nil selectedImageName:nil targe:self action:@selector(share)];
    ...
    self.navigationItem.rightBarButtonItems = @[shareBtn, fixedSpacer, collectionBtn];
}
...
- (void)share
{
    ...
    NSString *label = [NSString stringWithFormat:@"用户：%@ 分享了品牌：%@/brandId:%@", username, self.title, self.brand.brandId];
    [KPStatisticsTool event:@"brand_share_id" label:label];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 统计每个品牌被点击的次数
    NSString *label = [NSString stringWithFormat:@"品牌：%@,brandId:%@", self.title, self.brand.brandId];
    [KPStatisticsTool event:@"brand_id" label:label];
